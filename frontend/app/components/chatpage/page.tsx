    ...
    (pageType === 1 ? 
      <div className="flex flex-col h-screen items-center justify-center font-sans">
        <div className = "flex flex-col w-full items-center justify-center gap-4">
          ...
          <div className="flex items-center justify-center mt-4 w-full">
            <textarea 
              ref={inputRef1}
              className="w-1/2 placeholder:text-zinc-500 rounded-2xl p-3 border-2 border-zinc-300 focus:outline-none focus:border-y-cyan-500 focus:border-x-emerald-500" 
      ...
      <div className="flex flex-col items-center h-screen font-mono">
        ...
        <div className="fixed bg-slate-50/10 py-2 bottom-0 w-full flex items-center justify-center">
            <textarea ref={inputRef2} className="w-3/4 lg:w-1/2 bg-slate-700 rounded-2xl p-3 border-2 border-zinc-300 focus:outline-none focus:border-y-cyan-500 focus:border-x-emerald-500" placeholder="Type your message..." value={message||""} onChange={(e)=>setMessage(e.target.value)} onKeyDown={(e) => {if(e.key === "Enter" && !e.shiftKey) addToMessageList(e)}} />
            <button className="bg-emerald-600 transition-color duration-400 hover:bg-cyan-600 ml-2 h-12 w-12 rounded-full cursor-pointer" onClick={(e)=>{addToMessageList(e)}}><Image src="/submit.svg" width={80} height={80} className="p-2" alt="Send" /></button>
        </div>
      </div>
    )
